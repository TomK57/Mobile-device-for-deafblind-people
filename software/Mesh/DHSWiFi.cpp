...
//default IP-Address
IPAddress myIP(10, 0, 0, 10);  // empty start IP-Address
...
void WiFiInitAccessPoint() {
  uint8_t i = 0;
    ...
    if ((strlen(ssid) > 0) && (strncmp(ssid, "undefined", 32) != 0)) {
    ...
    do {
      ...
      i++;
    } while (i < WLANSEARCHTIME);  // try to connect for 60 seconds

    Serial.println();
  } else i = WLANSEARCHTIME;

  if (i == WLANSEARCHTIME) {                     // if no connection, switch to AP-mode
    WiFi.mode(WIFI_AP);
    mesh.apManual(); // disable scanning?
    WiFi.softAPConfig(myIP, myIP, IPAddress(255, 255, 255, 0));
    //delay(100);
    WiFi.softAP(DEVICE_NAME, MESH_PASSWORD, atoi(channel));
    ConnectType = 2;
  ...
  if (ConnectType == 1) oled.print("W");
  else if (ConnectType == 2) oled.print("A");
  else oled.print(" ");
...
/*  oled.setCursor(0, 16);     // set position to display
  oled.print("scan  WLAN");  // set text
  oled.display();            // display on OLED
  Serial.println("Scaning WLAN Network");
...
*/
  ...
  oled.setCursor(0, 16);     // set position to display
  oled.print("sear. Dev.");  // set text
  oled.display();            // display on OLED
  Serial.println("Searching for DeafBlind device MESH Network");
...
*/
  ...
  if ((ConnectType == 0) && (nodes.size() == 0)) {
    ...
    if ((now - startTime) > MESHSEARCHTIME*1000) {  // wait during start or when mesh connection is lost before WLAN/AP start
      WiFiInitAccessPoint();
      /*
...
void initServer() {
  ...
  });  // config page from flash (works with empty filesystem)
  server.on(
    "/Config.html", HTTP_POST, [](AsyncWebServerRequest *request) {
      request->send_P(200, "text/html", config_html, processor);
    },
    FileUpload);                                            // file upload data transfer
  ...
  server.on(
    "/rootupload", HTTP_POST, [](AsyncWebServerRequest *request) {
      request->send(200);
    },
    RootUpload);  //  handle upload own root firmware file
...
void handleCredentials(AsyncWebServerRequest *request) {
  ...
  if (request->hasParam("ssid", true)) {
    ...
    if (ssid == "default") {        // set back to defaults if given ssid = "default"
      ...
      if (request->hasParam("channel", true)) channel = request->getParam("channel", true)->value();
    }
    if (!EEPROM.begin(256)) {  // size in uint8_t
      Serial.println("Failed to initialise EEPROM");
    } else {
