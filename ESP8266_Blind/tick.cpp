...
void tickC::lineCommand(String c) { // process commands
  ...
  switch (c[0]) { // process command
    ...
    case 'o': if (!tickClient) for (int i=2; i<c.length(); i++) { processTick(c[i]); delay(outSpeed);} break;
    case 'n': if (!tickClient) { strlcpy(ssid,(char*)&c[2],32); Serial.printf("device Name: %s\n",ssid); //WiFi.setHostname(ssid); 
              MDNS.begin(ssid);} break; // change device name  
...
//              WiFi.setHostname(ap_ssid); 
    ...
    case 'l': u = 0; // send names of all active clients to world
              ...
              processTick(' '); // short pause between names
              delay(outSpeed);
